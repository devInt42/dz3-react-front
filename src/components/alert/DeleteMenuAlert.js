...
function DeleteMenuAlert(props) {
    ...
    MySwal.fire({
        ...
        if (result.isConfirmed) {
            ...
            axios.get('http://localhost:8080/menu/menulist/' + menuSequence).then(response => {
                if (response.data == 0) {
                    console.log(response.data);
                    props.deleteMenu(menuSequence);
                    Swal.fire('삭제가 완료되었습니다.', '', 'success', '#3085d6');
                } else { Swal.fire('삭제실패.', '하위 메뉴가 존재합니다. 하위메뉴부터 삭제해주세요.', 'error', '#3085d6'); console.log(response.data)}
