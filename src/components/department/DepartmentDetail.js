...
const DepartmentDetail = (props) => {
  ...
  const getWorkplace = async () => {
    try {
      const result = await axios.get(
        `${baseUrl}/department/workplace/${props.workplaceSeq}`
      );
  ...
  const getDepartment = async () => {
    try {
      const result = await axios.get(
        `${baseUrl}/department/list/${props.departmentSeq}`
      );
      setDepartment(result.data[0]);
    } catch (error) {
      console.log(error);
  ...
  useEffect(() => {
    departmentCall < 0
      ? setDepartmentCall("")
      : CallNumberCheck(departmentCall)
      ? setCallStyle(true)
      : setCallStyle(false);
  }, [departmentCall]);
  ...
  useEffect(() => {
    ...
    setDepartmentCall(
      department.departmentCall == null ? "" : department.departmentCall
    );
    setDepartmentFax(
      department.departmentFax == null ? "" : department.departmentFax
    );
  }, [department]);
  ...
  useEffect(() => {
    if (`${departmentCode}` == `${firstCode}` && props.departmentSeq !== 0) {
      ...
      axios
        .get(`${baseUrl}/department/info/check/`, {
          params: param,
        })
  ...
  useEffect(() => {
    if (departmentName === firstName) {
      ...
      axios
        .get(`${baseUrl}/department/info/namecheck`, {
          params: param,
        })
  ...
  const getDepartmentParent = async () => {
    try {
      const result = await axios.get(
        `${baseUrl}/department/list/${departmentParentSeq}`
      );
      ...
      <table id="department-table">
        ...
        <tbody>
            ...
            <td className="department-table-content">
              <div className="content-have-button">
                <Form.Control
                  ...
                  readOnly
                />
                <DepartmentParentModal
                  setDepartmentParentName={setDepartmentParentName}
                  workplaceSeq={props.workplaceSeq}
          ...
          <tr>
            ...
            <td className="department-table-content">
              <Form.Control
                ...
                isValid={
                  `${departmentCode}`.length < 1 || firstCode === departmentCode
                    ? false
                    : codeDupliCheck === 1
                    ? false
                    : true
          ...
          <tr>
            ...
            <td colSpan={3} className="department-table-content">
              <Form.Control
                ...
                isValid={
                  `${departmentName}`.length < 1 || firstName === departmentName
                    ? false
                    : nameDupliCheck === 1
                    ? false
                    : true
          ...
          <tr>
            ...
            <td className="department-table-content">
              <div className="company-table-td-twocontent">
                ...
                <Form.Control
                  ...
                  onChange={(e) => {
                    setDepartmentCall(CallNumber(areaCode + e.target.value));
                  }}
                  value={`${departmentCall}`.substring(areaCode.length) || ""}
                  isValid={callStyle}
                  isInvalid={
                    `${departmentCall}`.length < 1
                      ? ""
                      : callStyle
                      ? false
                      : true
            ...
            <td className="department-table-content">
              <Form.Control
                ...
                onChange={(e) => {
                  setDepartmentFax(FaxNumber(e.target.value));
                }}
                value={departmentFax || ""}
                isValid={faxStyle}
                isInvalid={
                  `${departmentFax}`.length < 1 ? "" : faxStyle ? false : true
                }
                style={{ zIndex: 0 }}
              />
            </td>
            ...
            <td colSpan={3} className="company-table-content">
              <div className="company-table-td-twocontent">
                <Form.Control
                  ...
                  onFocus={() => {
                    `${departmentZipCode}`.length > 0
                      ? setZipcodeIsOpen(false)
                      : setZipcodeIsOpen(true);
                  }}
                  isValid={
                    checked > 0 &&
                    `${departmentZipCode}`.length > 0 &&
                    departmentZipCode != null &&
                    departmentZipCode != undefined
                      ? true
                      : false
                  }
                  isInvalid={
              ...
              <div className="company-table-td-address">
                <div className="company-table-td-address-input">
                  <Form.Control
                    ...
                    isInvalid={
                      checked < 1 ? false : address.length > 0 ? false : true
                    }
                    readOnly
                  />
                </div>
  ...
  //유효성 검사를 하기 위한 함수
  function CallNumberCheck(value) {
    //대표번호 유효성 검사
  ...
  function FaxNumberCheck(value) {
    const check =
      /(^02[0-9]{0}|^01[0-9]{1}|050[0-9]{1}|[0-9]{3})-[0-9]{3,4}-[0-9]{4}$/;
    return check.test(value);
  }
};
