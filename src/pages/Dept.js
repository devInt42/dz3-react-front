    ...
    <div>
      {dept.map((dept, i) => {
        return (
          dept && (
            <div key={i}>
              <div>부서코드 : {dept.dept_code}</div>
              <br />
              <div>부서명 : {dept.dept_name}</div>
              <br />
              <div>위치 : {dept.location}</div>
              <br />
              <button
                onClick={() => {
                  deleteDept(dept.dept_code);
                }}
              >
                삭제
              </button>
      ...
      부서조회
      <input type="text" onChange={selectDeptCode} />
